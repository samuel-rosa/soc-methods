...
```{r}
full_cv$p <- rep(c("y ~ x1", "y ~ x1 + x2", "y ~ x1 + x2 + x3"), each = 24) %>% as.factor()
full_cv$m <- rep(c("Quantile (median) regression", "Weighted least squares regression"), nrow(full_cv) / 2)
full_cv$y <- rep(soil_vars[, 1], each = 2) %>% rep(3) %>% as.factor()
full_cv$x <- rep(soil_vars[, 2], each = 2) %>% rep(3) %>% as.factor()
...
xyplot(
  ...
  xlab = "Linear model formulation", scales = list(x = list(relation = "same"), y = list(relation = "sliced")),
