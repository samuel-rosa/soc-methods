...
for (k in 1:length(all_forms)) {
  ...
  for (j in 1:length(forms)) {
    ...
    for (i in 1:nrow(dataset)) {
      ...
      # out[i, ] <- 
      #   c(pred = exp(pred$pred + 0.5 * pred$pev), 
      #     pev = (exp(pred$pev) - 1) * exp(2 * pred$pred + pred$pev))
    }
    # Error statistics
    resid <- mean(y) - y
    error <- out$pred - y
...
xyplot(
  MedRSE ~ p, full_cv, groups = id, type = "l", col = col, lwd = 3, lty = lty,
...
p1 <-
xyplot(
  MedRSE + MRSE + mAVE + AVE + MSE + MAE + ME ~ as.factor(p), full_cv, 
  groups = id,  type = "l", 
  # col = col, lty = lty, 
  lwd = 3,
