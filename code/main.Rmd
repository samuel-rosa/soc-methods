...
We implement weighted least squares to estimate betas, make predictions and estimate prediction error variances.
Estimates are compared with the output of `lm` and `predict.lm`. This is usefull to understand the estimation
equations and their differences compared to ordinary least squares.
...
```{r}
# fit <- lm(tom ~ om + clay, data = camada[1:100, ], weights = 1/om)
# pred <- predict(fit, camada[101, ], weights = 1/om, se.fit = TRUE)
fit <- lm(tom ~ om + clay, data = camada[1:100, ])
pred <- predict.lm(fit, camada[i, ], se.fit = TRUE)
# summary(fit)

Y <- camada[1:100, "tom"] %>% as.matrix(ncol = 1)
X <- camada[1:100, c("b0", "om", "clay")] %>% as.matrix()
# W <- matrix(0, nrow = 100, ncol = 100)
# diag(W) <- c(1 / camada[1:100, "om"])[[1]]
b_hat <- solve(t(X)%*%X) %*% t(X) %*% Y %>% drop
x0 <- camada[i, c("b0", "om", "clay")] %>% as.matrix(ncol = 1, nrow = 3)
y_hat <- sum(x0 * t(b_hat))
sqrt(sqrt(sum(fit$residuals^2) / 97) * (1 + (x0 %*% solve(t(X)%*%X) %*% t(x0))));sqrt(pred$residual.scale^2 + pred$se.fit^2)
...
for (k in 1:length(all_forms)) {
  ...
  for (j in 1:length(forms)) {
    ...
    for (i in 1:nrow(dataset)) {
      ...
      err[i, "lm"] <- predict(lm_fit, dataset[i, ], weights = w[i]) - y[i]
    }
    # Error statistics
    resid <- mean(y) - y
    err_sqr <- err^2
