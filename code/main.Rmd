...
```{r}
fit <- gls(carbono_dry ~ mo_loi, camada)
plot(fit)
summary(fit)
```
...
```{r}
fit1 <- lme(carbono_dry ~ mo_loi, camada, random = ~ 1 | textura)
anova(fit, fit1)
plot(fit1, col = camada$textura)
```

```{r}
fit_varFixed <- update(fit1, weights = ~ mo_loi)
plot(fit_varFixed)
```

```{r}
fit_varPower <- update(fit1, weights = varPower(form = ~ mo_loi))
plot(fit_varPower)
```

```{r}
anova(fit_varFixed, fit_varPower)
anova(fit_varPower)
```
