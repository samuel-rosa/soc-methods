...
Define formulas. We test six model formulations, starting with predictor variables that are more readly 
available and ending with the most complex/expensive model. These are:

1. y ~ x
2. y ~ x + x^2
3. y ~ x + x^2 + topsoil
4. y ~ x + x^2 + topsoil + landuse
5. y ~ x*clay + x^2*clay + topsoil + landuse
6. y ~ x*clay + x^2*clay + topsoil + landuse + taxon
...
# Predictor variables
landuse <- "agriculture + grazing + forest"
taxon <- "argissolo + cambissolo + latossolo + neossolo + planossolo"

# Formulas
forms <- lapply(1:nrow(soil_vars), function (i) {
  y <- soil_vars[i, 1]
  x <- soil_vars[i, 2]
  list(
    glue("{y} ~ {x}"),
    glue("{y} ~ {x} + I({x}^2)"),
    glue("{y} ~ {x} + I({x}^2) + topsoil"),
    glue("{y} ~ {x} + I({x}^2) + topsoil + {landuse}"),
    glue("{y} ~ {x}*clay + I({x}^2)*clay + topsoil + {landuse}"),
    glue("{y} ~ {x}*clay + I({x}^2)*clay + topsoil + {landuse} + {taxon}")
    )
})
```
...
Test various predictor variables.

```{r, fig.asp=1}
# tom ~ oc; om ~ oc; toc ~ oc
# oc ~ toc; om ~ toc; tom ~ toc
# tom ~ om; toc ~ om; oc ~ om
# oc ~ tom; toc ~ tom; om ~ tom
fit_lm <- lm(toc ~ 0 + om, data = camada)
fit_lm0 <- lm(toc ~ om, data = camada)
fit_lm1 <- lm(toc ~ om + I(om^2), data = camada, weights = 1/toc)
fit_lm2 <- lm(toc ~ om + I(om^2) + topsoil + grazing + agriculture + forest, data = camada, weights = 1/toc)
fit_lm3 <- lm(toc ~ om*clay + I(om^2)*clay + topsoil + agriculture + forest, data = camada, weights = 1/toc)
fit_lm4 <- lm(toc ~ om*clay + I(om^2)*clay + topsoil + agriculture + forest + latossolo + argissolo + planossolo + cambissolo + neossolo, data = camada, weights = 1/toc)
# summary(fit_lm0);summary(fit_lm1);summary(fit_lm2);summary(fit_lm3);summary(fit_lm4)
plot(fit_lm, 1);plot(fit_lm0, 1);plot(fit_lm1, 1);plot(fit_lm2, 1);plot(fit_lm3, 1);plot(fit_lm4, 1)
```



```{r, fig.asp=1}
camada <- camada[order(camada$tom), ]
fit_lm1 <- 
  lm(tom ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada, weights = 1 / tom)
camada$y <- abs(resid(fit_lm1))
fit_lm <- lm(y ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada)
fit_lm2 <- 
  lm(tom ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada, weights = 1/abs(fitted(fit_lm1))^2)
fit_lm <- lm(y ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada)
camada$y <- abs(resid(fit_lm2))
fit_lm <- lm(y ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada)
fit_lm3 <- 
  lm(tom ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada, weights = 1/abs(fitted(fit_lm2))^2)
camada$y <- abs(resid(fit_lm3))
fit_lm <- lm(y ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada)
fit_lm4 <- 
  lm(tom ~ oc + I(oc^2) + clay + topsoil + agriculture, data = camada, weights = 1/abs(fitted(fit_lm3))^2)
plot(fit_lm1, 1);plot(fit_lm2, 1);plot(fit_lm3, 1);plot(fit_lm4, 1)
# lapply(list(fit_lm1, fit_lm2, fit_lm3, fit_lm4), function (x) coef(x) %>% signif(2))
summary(fit_lm1);summary(fit_lm4)
plot(weights(fit_lm1)/sum(weights(fit_lm1))*100);plot(weights(fit_lm2)/sum(weights(fit_lm2))*100);plot(weights(fit_lm3)/sum(weights(fit_lm3))*100);plot(weights(fit_lm4)/sum(weights(fit_lm4))*100)
```
