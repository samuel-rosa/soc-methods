...
camada <-
  camada %>% 
  filter(!is.na(carbono_fornalha_1min950_cgdct)) %>% # delete observations with NAs (densidade_aliquota...)
...
target <- c(1, 1, 0, 0, 0, qchisq(p = 0.5, df = 1))
ylim <- list(c(0.6417854, 1.01), c(0.6417854, 1.01), c(-0.001, 0.046221295), c(-0.00005, 2.034973e-03), 
             c(-0.02580879, 0.02289775), c(0.05160985, 0.60114634))
p1 <-
  xyplot(
    ...
    panel = function (...) {
      panel.grid(h = -1, v = -1)
      panel.abline(h = target[panel.number()], col = "red")
      panel.xyplot(...)
    })
p1$y.limits <- ylim
png(glue("../res/fig/cross-validation-wls.png"), width = 480 * 4, height = 480 * 5, res = 72 * 3)
p1
dev.off()
```
