...
forms <- 
  ...
  apply(1, function (x) list(reformulate(x[1], x[2]), reformulate(x[1])))
```

```{r, echo=FALSE}
# Calibrate models WITH intercept
fit_inter <- list()
for (i in 1:length(forms)) {
  fit_inter[[i]] <- gls(forms[[i]][[1]], data = camada, weights = varPower(form = forms[[i]][[2]]))
}
# lapply(fit_inter, function (x) summary(x)$tTable)
```

```{r}
# Calibrate models WITHOUT intercept
fit_nointer <- list()
for (i in 1:length(forms)) {
  up_form <- update(forms[[i]][[1]], ~ . - 1)
  fit_nointer[[i]] <- gls(up_form, data = camada, weights = varPower(form = forms[[i]][[2]]))
}
```
...
for (j in 1:length(forms)) {
  res <- data.frame(me = NA_real_, mae = NA_real_, mse = NA_real_, ave = NA_real_)
  err <- vector()
  for (i in 1:nrow(camada)) {
    y <- camada[as.character(forms[[j]][[1]][[2]])][[1]]
    resid <- mean(y) - y
    cv_fit <- gls(forms[[j]][[1]], data = camada[-i, ], weights = varPower(form = forms[[j]][[2]]))
    err[i] <- predict(cv_fit, camada[i, ]) - y[i]
  }
  res[, 1] <- mean(err)
  res[, 2] <- mean(abs(err))
  res[, 3] <- mean(err * err)
  res[, 4] <- 1 - (sum(err * err) / sum(resid * resid))
  cv[[j]] <- res
}
cv_inter <- do.call(rbind, cv) %>% round(4)
cv_inter
```
...
for (j in 1:length(forms)) {
  ...
  for (i in 1:nrow(camada)) {
    y <- camada[as.character(forms[[j]][[1]][[2]])][[1]]
    resid <- mean(y) - y
    up_form <- update(forms[[j]][[1]], ~ . - 1)
    cv_fit <- gls(up_form, data = camada[-i, ], weights = varPower(form = forms[[j]][[2]]))
    err[i] <- predict(cv_fit, camada[i, ]) - y[i]
  }
  res[, 1] <- mean(err)
  res[, 2] <- mean(abs(err))
...
cv_nointer <- do.call(rbind, cv) %>% round(4)
cv_nointer
```

```{r}
cbind(cv_inter$ave, cv_nointer$ave)
```
