...
```{r}
forms <- list(forms1, forms2, forms3)
...
full_cv$p <- rep(1:3, each = 24)
full_cv$m <- rep(c("rq", "lm"), nrow(full_cv) / 2)
full_cv$y <- rep(soil_vars[, 1], each = 2) %>% rep(3)
full_cv$x <- rep(soil_vars[, 2], each = 2) %>% rep(3)
full_cv$id <- rep(1:24, 3)
full_cv$id <- glue("{full_cv$y}~{full_cv$x}")
```

```{r}
xyplot(AVE ~ as.factor(p) | m, full_cv, groups = as.factor(id), type = "l")
```
