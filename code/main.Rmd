...
p1 <- 
  ...
  lattice::splom(
    groups = camada$textura, grid = TRUE, auto.key = list(columns = 3), xlab = '', aspect = "iso",
...
for (i in 1:length(formulas)) {
  ...
  for (j in 1:length(forms)) {
    ...
    fit[[j]] <- lm(formula = f, data = camada, weights = w)
    ...
    for (k in 1:nrow(camada)) {
      ...
      out[k, ] <- c(pred$fit, pred$se.fit^2 + pred$residual.scale^2 / w[k])
    }
...
cross_validation %>% 
  ...
  bwplot(MedSDR ~ p, data = ., 
         panel = function (x, y, ...) {
           panel.grid(v = 0, h = -1)
           panel.abline(h = qchisq(p = 0.5, df = 1), col = "red")
           panel.bwplot(x, y, ...)
           panel.xyplot(x, y, ...)
         })
```
...
png(glue("../res/fig/cross-validation-{estimator}2.png"), width = 480 * 4, height = 480 * 5, res = 72 * 3)
p1
dev.off()
```

Save fitted models and print model coefficients.
