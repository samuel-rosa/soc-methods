...
camada %>% 
  ...
  # Potential outliers
  # latticeExtra::layer(panel.text(x = x[c(58, 74)], y = y[c(58, 74)], c("58", "74"), pos = 1, cex = 0.7))
dev.off()
```
...
```{r, fig.asp=1}
observacao[74, ]
plot(dataset$oc ~ dataset$toc, type = "n")
text(y = dataset$oc, x = dataset$toc, labels = 1:105)
```
...
for (k in 1:length(all_forms)) {
  ...
  for (j in 1:length(forms)) {
    ...
    cv[[j]] <- data.frame(
      ...
      MAE = mean(error_abs),
      MSE = mean(error_sqr),
      # MRSE = mean(out$pev / error_sqr),
      MedRSE = qchisq(0.5, 1) + median(out$pev / error_sqr),
      AVE = 1 - (sum(error_sqr) / sum(resid * resid)),
      mAVE = 1 - (sum(error_abs) / sum(abs(resid)))
    )
  }
...
xyplot(
  ME ~ p, full_cv, groups = id, type = "l", col = col, lwd = 3, lty = lty,
