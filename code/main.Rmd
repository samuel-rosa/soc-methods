...
for (j in 1:length(forms)) {
  ...
  err <- vector()
  for (i in 1:nrow(camada)) {
    y <- camada[as.character(forms[[j]][[2]])][[1]]
    resid <- mean(y) - y
    up_form <- update(forms[[j]], ~ . - 1)
    # cv_fit <- gls(up_form, data = camada[-i, ], weights = varPower(form = forms[[j]][[2]]))
    cv_fit <- rq(up_form, data = camada[-i, ], method = 'fn')
    err[i] <- predict(cv_fit, camada[i, ]) - y[i]
  }
  res[, 1] <- median(err)
  res[, 2] <- median(abs(err))
  res[, 3] <- median(err * err)
...
```{r}
cbind(cv_inter$mse, cv_nointer$mse)
```
