...
```{r, echo=FALSE}
# Define formulas: Y = X1 + X2 + X3, where X2 = X1^2
forms <- c("toc", "oc", "om", "tom")
forms <- expand.grid(forms, forms)[, 2:1]
forms <- forms[!forms[, 1] == forms[, 2], ]
forms <- data.frame(y = forms[, 1], x1 = forms[, 2], x2 = glue("I({forms[, 2]}^2)"), x3 = "clay")
forms <- apply(forms, 1, function (x) reformulate(x[2:4], x[1]))
