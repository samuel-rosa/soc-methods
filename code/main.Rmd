...
camada <- 
  ...
  mutate(taxon = observacao$taxon_sibcs_2009)
```
...
fit_lm0 <- lm(toc ~ tom, data = camada)
fit_lm1 <- lm(toc ~ tom*clay + I(tom^2)*clay + topsoil + agriculture + forest, data = camada)
fit_lm2 <- lm(toc ~ oc*clay + I(oc^2)*clay + topsoil + agriculture + forest, data = camada, weights = 1/toc)
fit_lm3 <- lm(log(toc) ~ oc*clay + I(oc^2)*clay + topsoil + agriculture + forest, data = camada)
fit_lm4 <- lm(log(toc) ~ oc*clay + log(oc)*clay + topsoil + agriculture + forest, data = camada)
# summary(fit_lm0);summary(fit_lm1);summary(fit_lm2);summary(fit_lm3)
plot(fit_lm0, 1);plot(fit_lm1, 1);plot(fit_lm2, 1);plot(fit_lm3, 1);plot(fit_lm4, 1)
```
